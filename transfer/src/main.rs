...
async fn main() -> anyhow::Result<()> {
  ...
  // 分页参数
  let page_size = 10000;
  let mut last_id = 1917756202075178;

  let mut conn = mysql_pool.get_conn().await?;
  loop {
    // let sql = format!("SELECT * FROM vika_datasheet_record where dst_id=:dst_id LIMIT {} OFFSET {}", page_size, offset);
    ...
    mongodb_db
      .collection::<DataSheetRecord>("vika_datasheet_record")
